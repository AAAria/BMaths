...
# m.addConstr(quicksum(S[n] for n in N if n not in supply) == 1) $ Comm 7
m.addConstr(quicksum(S[n] for n in N if n not in supply and n != 10) == 1) # Comm 8
...
for t in T:
    ...
    for n in N:
        ...
        if n in supply:
            ...
            m.addConstr(Y[n,t] <= SSupply * S[n] * R[t]) # Comm 9 - running time constraint
